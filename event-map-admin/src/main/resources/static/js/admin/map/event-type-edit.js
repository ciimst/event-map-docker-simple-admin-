...
//		loadIcon(iconPath);
	...
	if ($("svg text")[0] && $("svg text").attr('x') == '50%') {
		$("svg text").attr('x', '25%')		
	}
	
	$("#IconCreatorBtn").on("click", function () { 
		var customIconText = $('#IconCreatorText').val()
		if (customIconText != "" && customIconText != undefined) {
			if (!$("svg text")[0]) {
				$("svg").append('<text x="25%" y="60%" text-anchor="middle" fill="#fff" font-size="35">'+ customIconText.toUpperCase() +'</text>')	
				$("svg").html($("svg").html());			
			}
			else 
			{
				$("svg text")[0].innerHTML = customIconText.toUpperCase();
			}
			$('#image').val('<text x="50%" y="60%" text-anchor="middle" fill="#fff" font-size="35">'+ customIconText.toUpperCase() +'</text>')
		}
	});
