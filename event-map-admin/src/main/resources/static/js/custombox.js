var CustomBox = function() {
	...
	return {
		...
		"alertWarning": function(message, alertFunction) {
			return bootbox.dialog({
				message:'<p class="text-danger text-center"><i class="fa-regular fa-circle-exclamation"></i></p><p class="text-center">{0}</p>'.f(message),
				buttons: {
					confirm: {
						label: '<i class="la la-fw la-check"></i> <span>{0}</span>'.f(lang.get("label.ok")),
						className: 'btn-primary',
		...
		"alertEventSuccess": function(message, alertFunction) {
			return bootbox.dialog({
				message: '<p class="text-success text-center"><i class="la la-fw la-5x la-check-circle"></i></p><p class="text-center">{0}</p>'.f(message) ,
				onEscape: function(e) {
					// you can do anything here you want when the user dismisses dialog
				...
				setTimeout(closeTimeout, 2000, $(this), alertFunction);
			});
		},
