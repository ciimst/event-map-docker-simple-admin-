let editUrl = contextPath + "admin/map/tile/edit";
let deleteUrl = contextPath + "admin/map/tile/delete";
let g_tableId = "#tileTableId";
...
$saveBtn.on("click", function (e) {
	...
	custombox.confirm(lang.get("label.are.you.sure.want.to.save"), function (result) {
		if (result) {
			...
			$.post(contextPath + "admin/map/tile/save", modalData)
			.fail(function (xhr) {
				xhr.state = false;
				console.error(xhr);
...
function openEditModal(elem) {
	...
	$.post(editUrl, {tileServerId: dataId})
	.fail(function (xhr) {
		xhr.state = false;
		console.error(xhr);
		xhr.description = lang.get("label.unknown.error");
...
function deleteItem(elem) {
	...
	$.post(deleteUrl, {tileServerId: dataId})
	.fail(function (xhr) {
		xhr.state = false;
		console.error(xhr);
		xhr.description = lang.get("label.unknown.error");
...
$editModal.on('show.bs.modal', function (event) {
	...
	if (type === "add") {
		...
		title = lang.get("label.tile.add");//TODO:lang
	} else {
		...
		modal.find('input#sortOrder').val(data.sortOrder);		
		modal.find('input#state').prop("checked", data.state);		
		title = lang.get("label.tile.edit");//TODO:lang
	}
